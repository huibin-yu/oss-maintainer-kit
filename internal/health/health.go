...
func Repository(root string) Summary {
	checks := []Check{
		...
		fileCheck(root, "govulncheck workflow", ".github/workflows/govulncheck.yml", "提供 Go 漏洞扫描"),
		fileCheck(root, "Scorecard workflow", ".github/workflows/scorecard.yml", "提供 OpenSSF Scorecard 安全治理评分"),
		...
		fileCheck(root, "Release artifacts workflow", ".github/workflows/release-artifacts.yml", "提供 tag 发布产物、SBOM 和 provenance"),
		...
		contentCheck(root, "govulncheck least privilege", ".github/workflows/govulncheck.yml", []string{"permissions:", "contents: read"}, "govulncheck workflow 使用最小只读权限"),
		contentCheck(root, "govulncheck package coverage", ".github/workflows/govulncheck.yml", []string{"golang/govulncheck-action", "go-version-input: \"1.21\"", "go-package: ./..."}, "govulncheck workflow 使用项目 Go 版本扫描全部 Go package"),
		contentCheck(root, "Scorecard security permission", ".github/workflows/scorecard.yml", []string{"permissions:", "contents: read", "security-events: write", "id-token: write"}, "Scorecard workflow 使用 SARIF 上传和结果发布所需最小权限"),
		contentCheck(root, "Scorecard SARIF upload", ".github/workflows/scorecard.yml", []string{"ossf/scorecard-action", "results_format: sarif", "publish_results: true", "github/codeql-action/upload-sarif"}, "Scorecard workflow 输出 SARIF 并上传 Code Scanning"),
		...
		contentCheck(root, "Release artifacts tag trigger", ".github/workflows/release-artifacts.yml", []string{"tags:", "\"v*\""}, "release-artifacts workflow 仅在版本 tag 上构建发布产物"),
		contentCheck(root, "Release artifacts provenance", ".github/workflows/release-artifacts.yml", []string{"attestations: write", "id-token: write", "actions/attest-build-provenance", "subject-path: \"dist/*\""}, "release-artifacts workflow 为发布产物生成 provenance attestation"),
		contentCheck(root, "Release artifacts SBOM checksums", ".github/workflows/release-artifacts.yml", []string{"sbom", "--output", "sha256sum", "actions/upload-artifact"}, "release-artifacts workflow 生成 SBOM、校验和并上传产物"),
