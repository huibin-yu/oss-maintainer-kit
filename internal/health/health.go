...
func Repository(root string) Summary {
	checks := []Check{
		...
		contentCheck(root, "Review diff Checks permission", ".github/workflows/review-diff.yml", []string{"checks: write"}, "PR diff workflow 具备 GitHub Checks 写权限", "为 review-diff workflow 添加 checks: write 权限。"),
		contentCheck(root, "Review diff Checks run", ".github/workflows/review-diff.yml", []string{"github-check-run", "github.event.pull_request.head.sha", "github.repository"}, "PR diff workflow 创建 GitHub Checks run", "在 review-diff workflow 中调用 github-check-run，把 diff 风险作为 GitHub Checks annotations 发布。"),
