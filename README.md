...
- `github-release`：把已合并 PR 生成的 release draft 创建为 GitHub Draft Release，避免维护者手工复制发布正文。
...
- `github-export`：从 GitHub REST 或 GraphQL API 分页导出 issues 或 PRs，支持状态和更新时间窗口过滤，便于接入真实仓库数据。
...
- `github-check-run`：把 PR diff 风险扫描结果创建为 GitHub Checks run，直接展示 annotations。
...
生成发布摘要和 Codex prompt：

```bash
go run ./cmd/oss-maintainer-kit release-summary \
  --input examples/pulls.json \
  --project oss-maintainer-kit \
  --version v0.1.0
```

调用 OpenAI-compatible provider 生成发布摘要：

```bash
OPENAI_API_KEY=sk_xxx go run ./cmd/oss-maintainer-kit release-summary \
  --input examples/pulls.json \
  --project oss-maintainer-kit \
  --version v0.1.0 \
...
创建 GitHub Checks run：

```bash
GITHUB_TOKEN=ghp_xxx go run ./cmd/oss-maintainer-kit github-check-run \
  --repo owner/name \
  --sha HEAD_SHA \
  --diff examples/pr.diff
```
